...
interface IVaultEvents {
    ...
    /**
     * @notice A new token was added to a pool.
     * @param pool The pool to which the token was added
     * @param token The address of the token added to the pool
     * @param tokenIndex The index of the token in the pool's token array
     * @dev This event is emitted when a new token is added to a pool, and it includes the pool address, the token address,
     * the index of the token in the pool's token array and the amount of BPT minted for the new token.
     */
    event AddedNewTokenToPool(address indexed pool, address indexed token, uint256 tokenIndex);
