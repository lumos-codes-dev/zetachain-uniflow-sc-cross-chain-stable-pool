        ...
        assembly {
            ...
            for { } lt(i, constructorArgsSize) { i := add(i, 32) } {
                ...
                if gt(add(i, 32), constructorArgsSize) {
                    copySize := sub(constructorArgsSize, i)
                }
                // Load word from source
                let data := mload(add(constructorArgsDataPtr, i))
                // If last word and not full 32 bytes, mask it
                if lt(copySize, 32) {
                    let mask := sub(shl(mul(8, sub(32, copySize)), 1), 1)
                    data := and(data, not(mask))
                    let destData := mload(add(constructorArgsCodeDataPtr, i))
                    data := or(and(destData, mask), data)
                }
                mstore(add(constructorArgsCodeDataPtr, i), data)
            }
        }
    }
